...
def load_config(data_folder):
    base_file = "/mmdetection/configs/cascade_rcnn/cascade_rcnn_r101_caffe_fpn_1x_coco.py"
    ...
    cfg.data.train.ann_file = 'gdsc_train_dataset.csv'  # the file containing the train data labels
    ...
    cfg.model.roi_head.bbox_head[0].num_classes = 1  # a worm section is the only object we are detecting
...
if __name__ == "__main__":
    ...
    prediction_df.to_csv(f'{output_folder}/results_6k_train_{epoch}.csv', sep=';')
    
    logging.info("Creating Test Predictions")
    file_names = pd.read_csv(f'{data_folder}/test_files.csv', sep=';', header=None)[0].values
    prediction_df = create_predictions(file_names, cfg, checkpoint, device='cuda')
    prediction_df.to_csv(f'{output_folder}/results_6k_test_{epoch}.csv', sep=';')
